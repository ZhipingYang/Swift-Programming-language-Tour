//
//  OFFCircleCountingAnimationView.swift
//  GetPlansCountingAnimation
//
...
private let ScreenHeight: CGFloat = 572

private let mainColor = UIColor.purple
private let lightBlueColor = UIColor.purple.withAlphaComponent(0.3)


public class OFFCircleCountingAnimationView: UIView, CAAnimationDelegate {
    ...
    lazy private var mainBackView: UIView = {
    ...
    lazy private var descripLabel: UILabel = {
    ...
    public class func showCountingAnimation(completion aCompletion: @escaping (Void)->(Void)) -> UIView {
        let view = OFFCircleCountingAnimationView(frame: CGRect(x: 0, y: 0, width: 320, height: 572))
    ...
    public func startShow() {
        ...
        let temporaryLayer = getCricleLayer(inview: mainView, radius: 135, borderWidth: 5, strokeColor: mainColor, startAngle: CGFloat(-M_PI_2*1.2), endAngle: CGFloat(-M_PI_2))
    ...
    private func thirdShow() {
        descripLabel.text = animationPeriod.description()
        DispatchQueue.main.asyncAfter(deadline: .now() + animationPeriod.duration()) {
            self.animationPeriod.next()
            self.endShow()
        }
    }
    ...
    private let interval = (AnimationPeriod.First.duration() + AnimationPeriod.Second.duration())/110.0
    
    private func percentLabelChange(percent: Int) {
        DispatchQueue.main.asyncAfter(deadline: .now() + interval) {
            if percent<100 {
                let newPercent = percent+1
                self.percentLabelChange(percent: newPercent)
                if percent == 30 {
                    self.secondShow()
                    self.animationPeriod.next()
                }
...
private extension OFFCircleCountingAnimationView {
